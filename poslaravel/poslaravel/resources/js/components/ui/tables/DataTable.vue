    ...
    <table class="w-full border border-gray-200 rounded-xl text-xs">
        <thead class="bg-gray-100 text-[var(--upme-dark-green)]">
            <tr>
                ...
                <th v-if="showActions && showEdit" class="border border-gray-300 px-2 py-2 w-12" />
                <th v-if="showActions && showDelete" class="border border-gray-300 px-2 py-2 w-12" />
            </tr>
        </thead>
        <tbody>
            <tr v-for="(item, index) in data" :key="index" class="hover:bg-gray-50">
                <td v-for="column in columns" :key="column.key" :class="[
                    'border border-gray-300 px-4 py-2 text-[var(--upme-dark-green)]',
                    getAlignClass(column.align)
                ]">
                ...
                <td v-if="showActions && showEdit"
                    class="border border-gray-300 px-2 py-2 text-center text-[var(--upme-dark-green)]">
                    <slot name="edit-button" :item="item" :index="index">
                        <ButtonIcon icon="fas fa-pencil" title="edit" class="text-[var(--upme-dark-green)]"
                            @click="handleEdit(item, index)" />
                    </slot>
                </td>
                <td v-if="showActions && showDelete" class="border border-gray-300 px-2 py-2 text-center">
